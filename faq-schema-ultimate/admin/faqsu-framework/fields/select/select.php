 ...
 *
 */
if ( ! class_exists( 'WPFAQSU_Field_select' ) ) {
  class WPFAQSU_Field_select extends WPFAQSU_Fields {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ! empty( $args['ajax'] ) ) {
          ...
          $args['settings']['data']['nonce'] = wp_create_nonce( 'wpfaqsu_chosen_ajax_nonce' );
        ...
        $chosen_sortable  = ( $args['chosen'] && $args['sortable'] ) ? ' wpfaqsu-chosen-sortable' : '';
        $chosen_ajax      = ( $args['chosen'] && $args['ajax'] ) ? ' wpfaqsu-chosen-ajax' : '';
        $placeholder_attr = ( $args['chosen'] && $args['placeholder'] ) ? ' data-placeholder="'. esc_attr( $args['placeholder'] ) .'"' : '';
        $field_class      = ( $args['chosen'] ) ? ' class="wpfaqsu-chosen'. esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) .'"' : '';
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. $field_name .'" class="wpfaqsu-hide-select hidden"'. $multiple_attr . $field_attr .'>';
