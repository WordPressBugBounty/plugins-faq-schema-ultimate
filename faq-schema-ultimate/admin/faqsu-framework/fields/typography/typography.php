 ...
 *
 */
if ( ! class_exists( 'WPFAQSU_Field_typography' ) ) {
  class WPFAQSU_Field_typography extends WPFAQSU_Fields {
    ...
    public function render() {
      ...
      $chosen_class     = ( $this->chosen ) ? ' wpfaqsu--chosen' : '';
      $line_height_unit = ( ! empty( $args['line_height_unit'] ) ) ? $args['line_height_unit'] : $args['unit'];

      echo '<div class="wpfaqsu--typography'. esc_attr( $chosen_class ) .'" data-depend-id="'. esc_attr( $this->field['id'] ) .'" data-unit="'. esc_attr( $args['unit'] ) .'" data-line-height-unit="'. esc_attr( $line_height_unit ) .'" data-exclude="'. esc_attr( $args['exclude'] ) .'">';

        echo '<div class="wpfaqsu--blocks wpfaqsu--blocks-selects">';
          ...
          if ( ! empty( $args['font_weight'] ) || ! empty( $args['font_style'] ) ) {
            ...
            echo '</select>';
            echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[font-weight]' ) ) .'" class="wpfaqsu--font-weight" value="'. esc_attr( $this->value['font-weight'] ) .'" />';
            echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[font-style]' ) ) .'" class="wpfaqsu--font-style" value="'. esc_attr( $this->value['font-style'] ) .'" />';
        ...
        echo '<div class="wpfaqsu--blocks wpfaqsu--blocks-inputs">';
        ...
        if ( ! empty( $args['preview'] ) ) {
          echo '<div class="wpfaqsu--block wpfaqsu--block-preview'. esc_attr( $always_preview ) .'">';
          echo '<div class="wpfaqsu--toggle fas fa-toggle-off"></div>';
          echo '<div class="wpfaqsu--preview">'. esc_attr( $args['preview_text'] ) .'</div>';
          echo '</div>';
        }

        echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[type]' ) ) .'" class="wpfaqsu--type" value="'. esc_attr( $this->value['type'] ) .'" />';
        echo '<input type="hidden" name="'. esc_attr( $this->field_name( '[unit]' ) ) .'" class="wpfaqsu--unit-save" value="'. esc_attr( $args['unit'] ) .'" />';
    ...
    public function create_select( $options, $name, $placeholder = '', $is_multiple = false ) {
      ...
      $output  = '<select name="'. esc_attr( $this->field_name( '['. $name .']'. $multiple_name ) ) .'" class="wpfaqsu--'. esc_attr( $name ) . esc_attr( $chosen_rtl ) .'" data-placeholder="'. esc_attr( $placeholder ) .'"'. $multiple_attr .'>';
    ...
    public function enqueue() {
      ...
      if ( ! wp_script_is( 'wpfaqsu-webfontloader' ) ) {
        ...
        $customwebfonts = apply_filters( 'wpfaqsu_field_typography_customwebfonts', array() );
        ...
        $defaultstyles = apply_filters( 'wpfaqsu_field_typography_defaultstyles', array( 'normal', 'italic', '700', '700italic' ) );

        $googlestyles = apply_filters( 'wpfaqsu_field_typography_googlestyles', array(
          '100'       => 'Thin 100',
    ...
    public function enqueue_google_fonts( $method = 'enqueue' ) {
      ...
      if ( ! empty( $this->value['type'] ) ) {
        ...
        WPFAQSU::include_plugin_file( 'fields/typography/google-fonts.php' );
        $is_google = ( array_key_exists( $this->value['font-family'], wpfaqsu_get_google_fonts() ) ) ? true : false;
      }
      ...
      if ( $is_google ) {
        ...
        if ( $font_weight || $font_style ) {
          ...
          if ( ! empty( $style ) ) {
            ...
            WPFAQSU::$webfonts[$method][$font_family][$style] = $style;
          }
        } else {
          WPFAQSU::$webfonts[$method][$font_family] = array();
        }
        ...
        if ( ! empty( $this->value['extra-styles'] ) ) {
          foreach ( $this->value['extra-styles'] as $extra_style ) {
            if ( ! empty( $extra_style ) ) {
              ...
              WPFAQSU::$webfonts[$method][$font_family][$extra_style] = $extra_style;
            }
          }
        }
