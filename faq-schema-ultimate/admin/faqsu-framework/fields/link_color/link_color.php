 ...
 *
 */
if ( ! class_exists( 'WPFAQSU_Field_link_color' ) ) {
  class WPFAQSU_Field_link_color extends WPFAQSU_Fields {
    ...
    public function render() {
      ...
      foreach ( $color_props as $color_prop_key => $color_prop_value ) {
        ...
        if ( ! empty( $args[$color_prop_key] ) ) {
          ...
          echo '<div class="wpfaqsu--left wpfaqsu-field-color">';
          echo '<div class="wpfaqsu--title">'. esc_attr( $color_prop_value ) .'</div>';
          echo '<input type="text" name="'. esc_attr( $this->field_name( '['. $color_prop_key .']' ) ) .'" value="'. esc_attr( $value[$color_prop_key] ) .'" class="wpfaqsu-color"'. $default_attr . $this->field_attributes() .'/>';
          echo '</div>';
