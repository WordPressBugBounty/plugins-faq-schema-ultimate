 ...
 *
 */
if ( ! class_exists( 'WPFAQSU_Shortcoder' ) ) {
  class WPFAQSU_Shortcoder extends WPFAQSU_Abstract{
    ...
    public function __construct( $key, $params = array() ) {
      ...
      add_action( 'wp_ajax_wpfaqsu-get-shortcode-'. $this->unique, array( $this, 'get_shortcode' ) );

      if ( ! empty( $this->args['show_in_editor'] ) ) {

        WPFAQSU::$shortcode_instances[$this->unique] = wp_parse_args( array( 'hash' => md5( $key ), 'modal_id' => $this->unique ), $this->args );

        // elementor editor support
        if ( WPFAQSU::is_active_plugin( 'elementor/elementor.php' ) ) {
          add_action( 'elementor/editor/before_enqueue_scripts', array( 'WPFAQSU', 'add_admin_enqueue_scripts' ) );
          add_action( 'elementor/editor/footer', array( 'WPFAQSU_Field_icon', 'add_footer_modal_icon' ) );
    ...
    public function add_footer_modal_shortcode() {
      ...
      $single_usage = ( ! $has_select ) ? ' wpfaqsu-shortcode-single' : '';
      $hide_header  = ( ! $has_select ) ? ' hidden' : '';

    ?>
      <div id="wpfaqsu-modal-<?php echo esc_attr( $this->unique ); ?>" class="wp-core-ui wpfaqsu-modal wpfaqsu-shortcode hidden<?php echo esc_attr( $single_usage . $class ); ?>" data-modal-id="<?php echo esc_attr( $this->unique ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'wpfaqsu_shortcode_nonce' ) ); ?>">
          ...
          <div class="wpfaqsu-modal-table-cell">
            ...
            <div class="wpfaqsu-modal-inner">
              <div class="wpfaqsu-modal-title">
                ...
                <div class="wpfaqsu-modal-close"></div>
              </div>
              <?php

                echo '<div class="wpfaqsu-modal-header'. esc_attr( $hide_header ) .'">';
                echo '<select>';
              ...
              <div class="wpfaqsu-modal-insert-wrapper hidden"><a href="#" class="button button-primary wpfaqsu-modal-insert"><?php echo $this->args['insert_title']; ?></a></div>
            </div>
          </div>
        </div>
      </div>
    ...
    public function get_shortcode() {
      ...
      if ( ! empty( $shortcode_key ) && wp_verify_nonce( $nonce, 'wpfaqsu_shortcode_nonce' ) ) {
        ...
        if ( ! empty( $section ) ) {
          ...
          if ( ! empty( $section['fields'] ) && $view !== 'repeater' ) {
            ...
            foreach ( $section['fields'] as $field ) {
              ...
              WPFAQSU::field( $field, $field_default, $shortcode, 'shortcode' );
          ...
          if ( ! empty( $repeatable_fields ) ) {
            ...
            echo '<div class="wpfaqsu--repeatable">';
              ...
              echo '<div class="wpfaqsu--repeat-shortcode">';

                echo '<div class="wpfaqsu-repeat-remove fas fa-times"></div>';
                ...
                foreach ( $repeatable_fields as $field ) {
                  ...
                  WPFAQSU::field( $field, $field_default, $inner_shortcode.'[0]', 'shortcode' );
            ...
            echo '<div class="wpfaqsu--repeat-button-block"><a class="button wpfaqsu--repeat-button" href="#"><i class="fas fa-plus-circle"></i> '. $button_title .'</a></div>';
    ...
    public static function once_editor_setup() {
      ...
      if ( function_exists( 'register_block_type' ) ) {
        add_action( 'enqueue_block_editor_assets', array( 'WPFAQSU_Shortcoder', 'add_guteberg_blocks' ) );
      }

      if ( wpfaqsu_wp_editor_api() ) {
        add_action( 'media_buttons', array( 'WPFAQSU_Shortcoder', 'add_media_buttons' ) );
      }
    ...
    public static function add_guteberg_blocks() {
      ...
      wp_enqueue_script( 'wpfaqsu-gutenberg-block', WPFAQSU::include_plugin_url( 'assets/js/gutenberg.js' ), $depends );

      wp_localize_script( 'wpfaqsu-gutenberg-block', 'wpfaqsu_gutenberg_blocks', WPFAQSU::$shortcode_instances );

      foreach ( WPFAQSU::$shortcode_instances as $value ) {

        register_block_type( 'wpfaqsu-gutenberg-block/block-'. $value['hash'], array(
          'editor_script' => 'wpfaqsu-gutenberg-block',
        ) );
    ...
    public static function add_media_buttons( $editor_id ) {
      ...
      foreach ( WPFAQSU::$shortcode_instances as $value ) {
        echo '<a href="#" class="button button-primary wpfaqsu-shortcode-button" data-editor-id="'. esc_attr( $editor_id ) .'" data-modal-id="'. esc_attr( $value['modal_id'] ) .'">'. $value['button_title'] .'</a>';
      }
