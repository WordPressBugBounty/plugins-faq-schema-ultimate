 ...
 *
 */
if ( ! class_exists( 'WPFAQSU_Options' ) ) {
  class WPFAQSU_Options extends WPFAQSU_Abstract {
    ...
    public $args         = array(
      ...
      // framework title
      'framework_title'         => 'FAQSU FRAMEWORK <small>by Codestar</small>',
      'framework_class'         => '',
    ...
    public function __construct( $key, $params = array() ) {
      ...
      $this->unique   = $key;
      $this->args     = apply_filters( "wpfaqsu_{$this->unique}_args", wp_parse_args( $params['args'], $this->args ), $this );
      $this->sections = apply_filters( "wpfaqsu_{$this->unique}_sections", $params['sections'], $this );
      ...
      add_action( 'wp_ajax_wpfaqsu_'. $this->unique .'_ajax_save', array( $this, 'ajax_save' ) );
    ...
    public function add_admin_bar_menu( $wp_admin_bar ) {
      ...
      if ( ! empty( $this->args['show_bar_menu'] ) && empty( $this->args['menu_hidden'] ) ) {
        ...
        $menu_icon = ( ! empty( $this->args['admin_bar_menu_icon'] ) ) ? '<span class="wpfaqsu-ab-icon ab-icon '. esc_attr( $this->args['admin_bar_menu_icon'] ) .'"></span>' : '';
    ...
    public function set_options( $ajax = false ) {
      ...
      // Set variables.
      $data      = array();
      $noncekey  = 'wpfaqsu_options_nonce'. $this->unique;
      $nonce     = ( ! empty( $response[$noncekey] ) ) ? $response[$noncekey] : '';
      $options   = ( ! empty( $response[$this->unique] ) ) ? $response[$this->unique] : array();
      $transient = ( ! empty( $response['wpfaqsu_transient'] ) ) ? $response['wpfaqsu_transient'] : array();

      if ( wp_verify_nonce( $nonce, 'wpfaqsu_options_nonce' ) ) {

        $importing  = false;
        $section_id = ( ! empty( $transient['section'] ) ) ? $transient['section'] : '';

        if ( ! $ajax && ! empty( $response[ 'wpfaqsu_import_data' ] ) ) {
        ...
        $data = apply_filters( "wpfaqsu_{$this->unique}_save", $data, $this );

        do_action( "wpfaqsu_{$this->unique}_save_before", $data, $this );
    ...
    public function save_options( $data ) {
      ...
      do_action( "wpfaqsu_{$this->unique}_saved", $data, $this );
    ...
    public function add_admin_footer_text() {
      $default = 'Thank you for creating with <a href="http://wpqode.com/" target="_blank">FAQSU FRAMEWORK</a>';
          ...
          foreach ( $sections['fields'] as $field ) {
            if ( ! empty( $field['id'] ) ) {
              if ( array_key_exists( $field['id'], $this->errors ) ) {
                $err = '<span class="wpfaqsu-label-error">!</span>';
              }
            }
          }
        }
    ...
    public function add_options_html() {
      ...
      $show_all      = ( ! $has_nav ) ? ' wpfaqsu-show-all' : '';
      $ajax_class    = ( $this->args['ajax_save'] ) ? ' wpfaqsu-save-ajax' : '';
      $sticky_class  = ( $this->args['sticky_header'] ) ? ' wpfaqsu-sticky-header' : '';
      $wrapper_class = ( $this->args['framework_class'] ) ? ' '. $this->args['framework_class'] : '';
      $theme         = ( $this->args['theme'] ) ? ' wpfaqsu-theme-'. $this->args['theme'] : '';
      $class         = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';
      $nav_type      = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';
      $form_action   = ( $this->args['form_action'] ) ? $this->args['form_action'] : '';

      do_action( 'wpfaqsu_options_before' );

      echo '<div class="wpfaqsu wpfaqsu-options'. esc_attr( $theme . $class . $wrapper_class ) .'" data-slug="'. esc_attr( $this->args['menu_slug'] ) .'" data-unique="'. esc_attr( $this->unique ) .'">';

        echo '<div class="wpfaqsu-container">';

        echo '<form method="post" action="'. esc_attr( $form_action ) .'" enctype="multipart/form-data" id="wpfaqsu-form" autocomplete="off" novalidate="novalidate">';

        echo '<input type="hidden" class="wpfaqsu-section-id" name="wpfaqsu_transient[section]" value="1">';

        wp_nonce_field( 'wpfaqsu_options_nonce', 'wpfaqsu_options_nonce'. $this->unique );

        echo '<div class="wpfaqsu-header'. esc_attr( $sticky_class ) .'">';
        echo '<div class="wpfaqsu-header-inner">';

          echo '<div class="wpfaqsu-header-left">';
          echo '<h1>'. $this->args['framework_title'] .'</h1>';
          echo '</div>';

          echo '<div class="wpfaqsu-header-right">';

            $notice_class = ( ! empty( $this->notice ) ) ? 'wpfaqsu-form-show' : '';
        ...
        echo '<div class="wpfaqsu-wrapper'. esc_attr( $show_all ) .'">';

          if ( $has_nav ) {

            echo '<div class="wpfaqsu-nav wpfaqsu-nav-'. esc_attr( $nav_type ) .' wpfaqsu-nav-options">';
              ...
              foreach ( $this->pre_tabs as $tab ) {
                ...
                $tab_icon  = ( ! empty( $tab['icon'] ) ) ? '<i class="wpfaqsu-tab-icon '. esc_attr( $tab['icon'] ) .'"></i>' : '';

                if ( ! empty( $tab['subs'] ) ) {

                  echo '<li class="wpfaqsu-tab-item">';

                    echo '<a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'" class="wpfaqsu-arrow">'. $tab_icon . $tab['title'] . $tab_error .'</a>';
                    ...
                    foreach ( $tab['subs'] as $sub ) {
                      ...
                      $sub_icon  = ( ! empty( $sub['icon'] ) ) ? '<i class="wpfaqsu-tab-icon '. esc_attr( $sub['icon'] ) .'"></i>' : '';
                  ...
                  echo '<li class="wpfaqsu-tab-item"><a href="#tab='. esc_attr( $tab_id ) .'" data-tab-id="'. esc_attr( $tab_id ) .'">'. $tab_icon . $tab['title'] . $tab_error .'</a></li>';
          ...
          echo '<div class="wpfaqsu-content">';

            echo '<div class="wpfaqsu-sections">';

            foreach ( $this->pre_sections as $section ) {

              $section_onload = ( ! $has_nav ) ? ' wpfaqsu-onload' : '';
              $section_class  = ( ! empty( $section['class'] ) ) ? ' '. $section['class'] : '';
              $section_icon   = ( ! empty( $section['icon'] ) ) ? '<i class="wpfaqsu-section-icon '. esc_attr( $section['icon'] ) .'"></i>' : '';
              ...
              echo '<div class="wpfaqsu-section hidden'. esc_attr( $section_onload . $section_class ) .'" data-section-id="'. esc_attr( $section_parent . $section_slug ) .'">';
              echo ( $has_nav ) ? '<div class="wpfaqsu-section-title"><h3>'. $section_icon . $section_title .'</h3></div>' : '';
              echo ( ! empty( $section['description'] ) ) ? '<div class="wpfaqsu-field wpfaqsu-section-description">'. $section['description'] .'</div>' : '';
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  WPFAQSU::field( $field, $value, $this->unique, 'options' );
          ...
          echo ( $has_nav && $nav_type === 'normal' ) ? '<div class="wpfaqsu-nav-background"></div>' : '';
      ...
      do_action( 'wpfaqsu_options_after' );
