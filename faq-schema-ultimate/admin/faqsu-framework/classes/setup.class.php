...
if ( ! class_exists( 'WPFAQSU' ) ) {
  class WPFAQSU {
    ...
    public function __construct() {
      ...
      // Init action
      do_action( 'wpfaqsu_init' );

      // Setup textdomain
      self::textdomain();

      add_action( 'after_setup_theme', array( 'WPFAQSU', 'setup' ) );
      add_action( 'init', array( 'WPFAQSU', 'setup' ) );
      add_action( 'switch_theme', array( 'WPFAQSU', 'setup' ) );
      add_action( 'admin_enqueue_scripts', array( 'WPFAQSU', 'add_admin_enqueue_scripts' ) );
      add_action( 'wp_enqueue_scripts', array( 'WPFAQSU', 'add_typography_enqueue_styles' ), 80 );
      add_action( 'wp_head', array( 'WPFAQSU', 'add_custom_css' ), 80 );
      add_filter( 'admin_body_class', array( 'WPFAQSU', 'add_admin_body_class' ) );
    ...
    public static function setup() {
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Options' ) && ! empty( self::$args['admin_options'] ) ) {
        foreach ( self::$args['admin_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Customize_Options' ) && ! empty( self::$args['customize_options'] ) ) {
        foreach ( self::$args['customize_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Customize_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Metabox' ) && ! empty( self::$args['metabox_options'] ) ) {
        foreach ( self::$args['metabox_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Nav_Menu_Options' ) && ! empty( self::$args['nav_menu_options'] ) ) {
        foreach ( self::$args['nav_menu_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Nav_Menu_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Profile_Options' ) && ! empty( self::$args['profile_options'] ) ) {
        foreach ( self::$args['profile_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Profile_Options::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Taxonomy_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
        ...
        foreach ( self::$args['taxonomy_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Taxonomy_Options::instance( $key, $params );
      ...
      if ( class_exists( 'WPFAQSU_Widget' ) && class_exists( 'WP_Widget_Factory' ) && ! empty( self::$args['widget_options'] ) ) {
        ...
        foreach ( self::$args['widget_options'] as $key => $value ) {
          if ( ! isset( self::$inited[$key] ) ) {
            ...
            self::$inited[$key] = true;
            $wp_widget_factory->register( WPFAQSU_Widget::instance( $key, $value ) );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Comment_Metabox' ) && ! empty( self::$args['comment_options'] ) ) {
        foreach ( self::$args['comment_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Comment_Metabox::instance( $key, $params );
      ...
      $params = array();
      if ( class_exists( 'WPFAQSU_Shortcoder' ) && ! empty( self::$args['shortcode_options'] ) ) {
        foreach ( self::$args['shortcode_options'] as $key => $value ) {
          if ( ! empty( self::$args['sections'][$key] ) && ! isset( self::$inited[$key] ) ) {
            ...
            WPFAQSU_Shortcoder::instance( $key, $params );

          }
        }

        // Once editor setup for gutenberg and media buttons
        if ( class_exists( 'WPFAQSU_Shortcoder' ) && ! empty( self::$shortcode_instances ) ) {
          foreach ( self::$shortcode_instances as $instance ) {
            if ( ! empty( $instance['show_in_editor'] ) ) {
              WPFAQSU_Shortcoder::once_editor_setup();
              break;
            }
          }
        }
    ...
    public static function include_plugin_file( $file, $load = true ) {
      ...
      $path     = '';
      $file     = ltrim( $file, '/' );
      $override = apply_filters( 'wpfaqsu_override', 'wpfaqsu-override' );
    ...
    public static function includes() {
      ...
      $fields = apply_filters( 'wpfaqsu_fields', array(
        'accordion',
        'background',
        'backup',
        'border',
      ...
      if ( ! empty( $fields ) ) {
        foreach ( $fields as $field ) {
          if ( ! class_exists( 'WPFAQSU_Field_'. $field ) && class_exists( 'WPFAQSU_Fields' ) ) {
    ...
    public static function add_admin_enqueue_scripts() {
      ...
      if ( $wpscreen->id === 'tools_page_wpfaqsu-welcome' ) {
        self::$enqueue = true;
      }

      if ( ! apply_filters( 'wpfaqsu_enqueue_assets', self::$enqueue ) ) {
        return;
      }
      ...
      if ( apply_filters( 'wpfaqsu_fa4', false ) ) {
        wp_enqueue_style( 'wpfaqsu-fa', 'https://cdn.jsdelivr.net/npm/font-awesome@4.7.0/css/font-awesome'. $min .'.css', array(), '4.7.0', 'all' );
      } else {
        wp_enqueue_style( 'wpfaqsu-fa5', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/all'. $min .'.css', array(), '5.15.5', 'all' );
        wp_enqueue_style( 'wpfaqsu-fa5-v4-shims', 'https://cdn.jsdelivr.net/npm/@fortawesome/fontawesome-free@5.15.4/css/v4-shims'. $min .'.css', array(), '5.15.5', 'all' );
      }
      ...
      // Main RTL styles
      if ( is_rtl() ) {
        wp_enqueue_style( 'wpfaqsu-rtl', self::include_plugin_url( 'assets/css/style-rtl'. $min .'.css' ), array(), self::$version, 'all' );
      }
      ...
      if ( ! empty( self::$fields ) ) {
        foreach ( self::$fields as $field ) {
          if ( ! empty( $field['type'] ) ) {
            $classname = 'WPFAQSU_Field_' . $field['type'];
    ...
    public static function add_typography_enqueue_styles() {
      ...
      if ( ! empty( self::$webfonts ) ) {
        ...
        if ( ! empty( self::$webfonts['enqueue'] ) ) {
          ...
          wp_enqueue_style( 'wpfaqsu-google-web-fonts', esc_url( add_query_arg( $query, '//fonts.googleapis.com/css' ) ), array(), null );
        ...
        if ( ! empty( self::$webfonts['async'] ) ) {
          ...
          wp_enqueue_script( 'wpfaqsu-google-web-fonts', esc_url( '//ajax.googleapis.com/ajax/libs/webfont/1.6.26/webfont.js' ), array(), null );

          wp_localize_script( 'wpfaqsu-google-web-fonts', 'WebFontConfig', array( 'google' => array( 'families' => $fonts ) ) );
    ...
    public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
      ...
      $is_pseudo  = ( ! empty( $field['pseudo'] ) ) ? ' wpfaqsu-pseudo-field' : '';
      ...
      if ( ! empty( $field['dependency'] ) ) {
        ...
        $visible = ( ! empty( $depend_visible ) ) ? ' wpfaqsu-depend-visible' : ' wpfaqsu-depend-hidden';

      }

      // These attributes has been sanitized above.
      echo '<div class="wpfaqsu-field wpfaqsu-field-'. $field_type . $is_pseudo . $class . $visible .'"'. $depend .'>';

      if ( ! empty( $field_type ) ) {

        if ( ! empty( $field['fancy_title'] ) ) {
          echo '<div class="wpfaqsu-fancy-title">' . $field['fancy_title'] .'</div>';
        }

        if ( ! empty( $field['title'] ) ) {
          echo '<div class="wpfaqsu-title">';
          echo '<h4>'. $field['title'] .'</h4>';
          echo ( ! empty( $field['subtitle'] ) ) ? '<div class="wpfaqsu-subtitle-text">'. $field['subtitle'] .'</div>' : '';
          echo '</div>';
        }

        echo ( ! empty( $field['title'] ) || ! empty( $field['fancy_title'] ) ) ? '<div class="wpfaqsu-fieldset">' : '';

        $value = ( ! isset( $value ) && isset( $field['default'] ) ) ? $field['default'] : $value;
        $value = ( isset( $field['value'] ) ) ? $field['value'] : $value;

        $classname = 'WPFAQSU_Field_'. $field_type;
